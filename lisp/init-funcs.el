...
;; Browse URL
(defun danis-webkit-browse-url (url &optional pop-buffer new-session)
...
(defun reload-init-file ()
  ...
  (interactive)
  (load user-init-file))
(defalias 'danis-reload-init-file #'reload-init-file)

;; Browse the homepage
(defun browse-homepage ()
  "Browse the Github page of DNR Emacs."
  (interactive)
  (browse-url danis-homepage))
...
(defun find-custom-file()
  ...
  (unless (file-exists-p custom-file)
    (if (file-exists-p danis-custom-example-file)
        (copy-file danis-custom-example-file custom-file)
      (user-error "The file `%s' doesn't exist" danis-custom-example-file)))
  (when (file-exists-p custom-file)
    (find-file custom-file))
  (when (file-exists-p danis-custom-post-file)
    (find-file-other-window danis-custom-post-file)))
...
(defun danis-treesit-available-p ()
  "Check whether tree-sitter is available.
Native tree-sitter is introduced since 29.1."
  (and danis-tree-sitter
       (fboundp 'treesit-available-p)
       (treesit-available-p)))

(defun danis-set-variable (variable value &optional no-save)
...
(define-minor-mode danis-read-mode
   ...
   (list
    (intern
     (completing-read "Select package archives: "
                      (mapcar #'car danis-package-archives-alist)))))
  ;; Set option
  (danis-set-variable 'danis-package-archives archives no-save)

  ;; Refresh if need
  (and refresh (package-refresh-contents async))

  (message "Set package archives to `%s'" archives))
(defalias 'danis-set-package-archives #'set-package-archives)

;; Refer to https://emacs-china.org/t/elpa/11192
(defun danis-test-package-archives (&optional no-chart)
  ...
  (let* ((durations (mapcar
    ...
    (when (and (not no-chart)
      ...
      (chart-bar-quickie
       ...
       (mapcar (lambda (p) (symbol-name (car p))) danis-package-archives-alist)
       "ELPA"
...
;; Update
(defun update-config ()
  "Update DNR Emacs configurations to the latest version."
  (interactive)
...
(defalias 'danis-update-config #'update-config)
...
(defalias 'danis-update-packages #'update-packages)
...
(defun update-config-and-packages()
  ...
  (update-config)
  (update-packages))
(defalias 'danis-update #'update-config-and-packages)
...
(defalias 'danis-update-dotfiles #'update-dotfiles)
...
(defun update-org ()
  ...
  (let ((dir (expand-file-name "~/org/")))
    (if (file-exists-p dir)
      ...
      (message "\"%s\" doesn't exist" dir))))
(defalias 'danis-update-org #'update-org)
...
(defalias 'danis-update-all #'update-all)


;; Fonts
(defun danis-install-fonts ()
...
(defun childframe-completion-workable-p ()
  ...
  (and (eq danis-completion-style 'childframe)
       (childframe-workable-p)))

(defun danis--theme-name (theme)
  "Return internal THEME name."
  (or (alist-get theme danis-theme-alist) theme 'doom-one))

(defun danis-compatible-theme-p (theme)
  "Check if the THEME is compatible. THEME is a symbol."
  (or (memq theme '(auto random system))
      (string-prefix-p "doom" (symbol-name (danis--theme-name theme)))))

(defun danis-dark-theme-p ()
  "Check if the current theme is a dark theme."
  (eq (frame-parameter nil 'background-mode) 'dark))

(defun danis-theme-enable-p (theme)
  "The THEME is enabled or not."
  (and theme
       (not (memq danis-theme '(auto random system)))
       (memq (danis--theme-name theme) custom-enabled-themes)))

(defun danis--load-theme (theme)
  "Disable others and enable new THEME."
  (when-let ((theme (danis--theme-name theme)))
    (mapc #'disable-theme custom-enabled-themes)
    (load-theme theme t)))

(defun danis--load-system-theme (appearance)
  "Load theme, taking current system APPEARANCE into consideration."
  (danis--load-theme (alist-get appearance danis-system-themes)))

(defun danis-load-random-theme ()
  "Load the random theme."
  (interactive)
  (let* ((themes (mapcar #'cdr danis-theme-alist))
         (theme (nth (random (length themes)) themes)))
    (if (eq theme danis-theme)
        (danis-load-random-theme)
      (danis--load-theme theme))))

(defun danis-load-theme (theme &optional no-save)
  ...
  (pcase theme
    ('auto
     ...
     (use-package circadian
       ...
       :custom (circadian-themes danis-auto-themes)
    ...
    ('system
     ...
     (use-package auto-dark
       ...
       :init
       (setq auto-dark-light-theme (alist-get 'light danis-system-themes)
             auto-dark-dark-theme (alist-get 'dark danis-system-themes))
  ...
  ;; Set option
  (danis-set-variable 'danis-theme theme no-save))

(advice-add #'consult-theme :after
            (lambda (theme)
              "Save theme."
              (danis-set-variable 'danis-theme theme)))



;; Frame
(defvar danis-frame--geometry nil)
(defun danis-frame--save-geometry ()
  "Save current frame's geometry."
  (setq danis-frame--geometry
...
(defun danis-frame--fullscreen-p ()
  "Return Non-nil if the frame is fullscreen."
  (memq (frame-parameter nil 'fullscreen) '(fullscreen fullboth)))

(defun danis-frame-maximize ()
  "Maximize the frame."
  (interactive)
  (danis-frame--save-geometry)
  (unless (eq (frame-parameter nil 'fullscreen) 'maximized)
    (set-frame-parameter nil 'fullscreen 'maximized)))

(defun danis-frame-restore ()
  "Restore the frame's size and position."
  (interactive)
  (modify-frame-parameters nil danis-frame--geometry))

(defun danis-frame-left-half ()
  "Put the frame to the left-half."
  (interactive)
  (unless (danis-frame--fullscreen-p)
    (danis-frame--save-geometry)
...
(defun show-http-proxy ()
  ...
  (if url-proxy-services
      (message "Current HTTP proxy is `%s'" danis-proxy)
...
(defun enable-socks-proxy ()
  ...
  (let* ((proxy (split-string danis-socks-proxy ":"))
         (host (car proxy))
         (port (string-to-number (cadr proxy))))
    (setq socks-server `("Default server" ,host ,port 5)))
  (setenv "all_proxy" (concat "socks5://" danis-socks-proxy))
  (show-socks-proxy))
